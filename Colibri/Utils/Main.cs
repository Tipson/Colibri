...
public class mainFuncs
{
    ...
    // {
    //     
    //     if (HttpContextHelper.Current.Request.Headers["User-Agent"].Any())
    //     {
    //         var u = HttpContextHelper.Current.Request.Headers["User-Agent"].ToString();
    //
    //         if (u.Length < 4)
    //             return false;
    //
    ...
    public static string GetLanguageFromUrl(string url, string lang)
    {
        string returnUrl = url.Trim();
        ...
        if (returnUrl.Length <= 4 && lang != "en")
        {
            returnUrl = "/" + lang;
        }
        else if (returnUrl.Length <= 4 && lang == "en")
        {
            returnUrl = "/";
        }
        else
        {
            returnUrl = "/" + lang + returnUrl.Substring(3, returnUrl.Length - 3);
        ...
        returnUrl = "https://www.colibri-group.org/" + returnUrl;
        return returnUrl;
    }

    public static string GenerateCode(int symbCount)
    {
        string s0, s1, st;
        s0 = "";
